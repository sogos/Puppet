...
class apache::base {
  ...
  include apache::params
	  ...
	  firewall { '031 Allow users to connect to Apache on port 80 but from local iface only':
		      proto => 'tcp',
                      port => '80',
                      #iniface => 'eth0',
                      action => "accept",
  ...
  apache::module {["alias", "auth_basic", "authn_file", "authz_default", "authz_groupfile", "headers", "authz_host", "authz_user", "autoindex", "dir", "env", "mime", "negotiation", "rewrite", "setenvif", "status", "cgi"]:
    ensure => present,
